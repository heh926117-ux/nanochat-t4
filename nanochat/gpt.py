...
class CausalSelfAttention(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        # Fused QKV projection: one matmul instead of three. This is useful on
        # small GPUs such as the T4, where kernel-launch overhead is significant.
        self.c_qkv = Linear(self.n_embd, self.n_head * self.head_dim + 2 * self.n_kv_head * self.head_dim, bias=False)
...
class GPT(nn.Module):
    ...
    def init_weights(self):
        ...
        for block in self.transformer.h:
            torch.nn.init.uniform_(block.attn.c_qkv.weight, -s, s) # fused QKV weights use Uniform to avoid outliers
        ...
        # Smear scalar and gate must be explicitly initialized
    ...
    def num_scaling_params(self):
        ...
        scalars = self.resid_lambdas.numel() + self.x0_lambdas.numel() + self.smear_gate.weight.numel() + self.smear_lambda.numel()
